...
| baseline.py | Polynomial NaN-safe via finite mask; pybaselines path requires all-finite (documented). Unknown method → ValueError. | OK / larger: consider raising ProcessingError from baseline_correct directly (currently re-wrapped at op_baseline). |
...
3. baseline_correct still raises bare ValueError/ImportError when called outside op_baseline.
