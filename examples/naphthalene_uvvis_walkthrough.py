"""Naphthalene UV-Vis walkthrough — analysis path demo (not compound ID).

Runnable twin of ``naphthalene_uvvis_walkthrough.ipynb``. Uses the NIST Chemistry
WebBook naphthalene UV-Vis JCAMP in ``fixtures/public/``.
...
filename and NIST title say “Naphthalene” for provenance only.
...
FIXTURE_REL = Path("fixtures") / "public" / "naphthalene_uvvis_nist.jdx"
SOURCES_REL = Path("fixtures") / "public" / "SOURCES.md"
DEFAULT_PROMINENCE = 0.1  # vibronic structure after poly baseline (benzene twin; acetone uses 0.05)


def find_repo_root(start: Path | None = None) -> Path:
    """Locate repo root by presence of the public naphthalene UV-Vis JCAMP fixture."""
...
def run(
    *,
    ...
    """Load naphthalene UV-Vis → baseline → smooth → peaks → optional export.
    ...
    ax.set_title(
        "ChemSpec walkthrough — public naphthalene UV-Vis fixture "
    ...
    if save_dir is not None:
        ...
        fig_path = save_dir / "naphthalene_uvvis_walkthrough.png"
        fig.savefig(fig_path, dpi=120)
        out["figure"] = str(fig_path)

        csv_path = save_dir / "naphthalene_uvvis_peaks.csv"
        peaks_to_csv(peaks, csv_path)
        out["peaks_csv"] = str(csv_path)

        session_path = save_dir / "naphthalene_uvvis_walkthrough.csw.json"
        save_session(
            session_path,
            raw,
            history=history,
            peaks=peaks,
            notes=(
                "Naphthalene UV-Vis tutorial walkthrough. "
...
def _print_summary(result: dict) -> None:
    print(
        "ChemSpec — naphthalene UV-Vis walkthrough "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Naphthalene UV-Vis analysis walkthrough (not compound ID)"
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
