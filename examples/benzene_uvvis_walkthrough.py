"""Benzene UV-Vis walkthrough — analysis path demo (not compound ID).

Runnable twin of ``benzene_uvvis_walkthrough.ipynb``. Uses the NIST Chemistry
WebBook benzene UV-Vis JCAMP in ``fixtures/public/``.
...
filename and NIST title say “Benzene” for provenance only.
...
FIXTURE_REL = Path("fixtures") / "public" / "benzene_uvvis_nist.jdx"
SOURCES_REL = Path("fixtures") / "public" / "SOURCES.md"
DEFAULT_PROMINENCE = 0.1


def find_repo_root(start: Path | None = None) -> Path:
    """Locate repo root by presence of the public benzene UV-Vis JCAMP fixture."""
...
def run(
    *,
    ...
    """Load benzene UV-Vis → baseline → smooth → peaks → optional export.
    ...
    ax.set_title(
        "ChemSpec walkthrough — public benzene UV-Vis fixture "
    ...
    if save_dir is not None:
        ...
        fig_path = save_dir / "benzene_uvvis_walkthrough.png"
        fig.savefig(fig_path, dpi=120)
        out["figure"] = str(fig_path)

        csv_path = save_dir / "benzene_uvvis_peaks.csv"
        peaks_to_csv(peaks, csv_path)
        out["peaks_csv"] = str(csv_path)

        session_path = save_dir / "benzene_uvvis_walkthrough.csw.json"
        save_session(
            session_path,
            raw,
            history=history,
            peaks=peaks,
            notes=(
                "Benzene UV-Vis tutorial walkthrough. "
...
def _print_summary(result: dict) -> None:
    print(
        "ChemSpec — benzene UV-Vis walkthrough "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Benzene UV-Vis analysis walkthrough (not compound ID)"
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
